...
int get_port(const char *portstring, const char *caller) {
	...
	if ((l = strtol(portstring, NULL, 10)) <= 0) {
		fprintf(stdout, "[ERROR] [%s] Invalid port: %s\n", caller, portstring);
		return -1;
	} else if (l < 1024) {
		fprintf(stdout, "[ERROR] [%s] Reserved port (value cannot be less than 1024): %s\n", caller, portstring);
		return -1;
	} else if (l > 65535) {
		fprintf(stdout, "[ERROR] [%s] Invalid port (value must be less than or equal to 65535): %s\n", caller, portstring);
		return -1;
	}
	fprintf(stdout, "[LOG] [RECEIVER] Port initialised\n");
	return (int) l;
}
...
void send_acknowledgment(int sfd, pkt_t *packet, uint8_t wdw, uint8_t type) {
	...
	if (write(sfd, lastack, 12) < 0) {
		fprintf(stdout, "[ERROR] [RECEIVER] Error while sending (N)ACK for packet %d\n", seqnum);
	} else {
		fprintf(stdout, "[LOG] [RECEIVER] (N)ACK sent for %d w/ seqnum %d\n", seqnum, succ(seqnum));
	}
...
int write_data(FILE *f, pkt_t *packet) {
	fprintf(stdout, "[LOG] [RECEIVER] Writing payload for packet %d to file\n", pkt_get_seqnum(packet));
	size_t write = fwrite(pkt_get_payload(packet), sizeof(char), pkt_get_length(packet), f);
	if (write != pkt_get_length(packet)) {
		fprintf(stdout, "[ERROR] [RECEIVER] Error while writing payload for packet %d to file\n", pkt_get_seqnum(packet));
		return -1;
	}
	return 0;
}
...
static void receive_data(FILE *f, int sfd) {
	...
	if (errc != 0) {
		fprintf(stdout, "[LOG] [RECEIVER] Error with dc timer\n");
		return;
	}
	...
	if (errcdef != 0) {
		fprintf(stdout, "[LOG] [RECEIVER] Error with rts timer\n");
		return;
	}
	...
	while (cont) {
		...
		if (bytes_read == -1) {
			fprintf(stdout, "[ERROR] [RECEIVER] Invalid read from socket\n");
			continue;
		}
		...
		if (errcd != 0) {
			fprintf(stdout, "Error with dc timer\n");
			return;
		}
		// disconnect if no transmission for a long period of time
		int timeout = 40;
		if (timee.tv_sec - last_time.tv_sec > timeout) {
			fprintf(stdout, "[LOG] [RECEIVER] No transmission for %d s; disconnect\n", timeout);
			return;
		}
		last_time = timee;
		...
		if(err!=0){
			fprintf(stdout, "error clock in sending data");
			return;
		}
		...
		if (pkt_status == PKT_OK) {
			...
			if ((seqnum > last_seqnum && seqnum < last_seqnum + window_size + 1) || ((last_seqnum + window_size > 255) && ((seqnum > last_seqnum) || (seqnum < (last_seqnum + window_size + 1) % 256)))) {
				...
				while (!minq_empty(pkt_queue) && ((pkt_t *) minq_peek(pkt_queue))->seqnum == succ(last_seqnum)) {
					...
					if (pkt_get_length(packet) == 0) {
						fprintf(stdout, "[LOG] [RECEIVER] Terminating packet received with seqnum %d (window starts at %d and has length %d)\n", pkt_get_seqnum(packet), last_seqnum, window_size);
						cont = false;
					} else {
						fprintf(stdout, "[LOG] [RECEIVER] Data packet %d received\n", pkt_get_seqnum(packet));
				...
				fprintf(stdout, "[LOG] [RECEIVER] Sequence number %d not in window with start at %d and size %d\n", pkt_get_seqnum(packet), last_seqnum+1, window_size);
				if (lastack != NULL) {
			...
			if (pkt_status == E_TR && pkt_get_type(packet) == PTYPE_DATA) {
				...
				fprintf(stdout, "[LOG] [RECEIVER] Packet %d truncated", pkt_get_seqnum(packet));
			} else {
				fprintf(stdout, "[LOG] [RECEIVER] Packet status not OK (error code %d) for packet %d \n", pkt_status, pkt_get_seqnum(packet));
			}
			pkt_del(packet);
		}
	}
...
int main(int argc, char *argv[]) {
		...
		while ((opt = getopt(argc, argv, "f:")) != -1) {
			switch (opt) {
				case 'f':
					...
					file_specified = 1;
					fprintf(stdout, "[LOG] [RECEIVER] File specified: %s\n", fn);
					break;
				default:
					return EXIT_FAILURE;
			}
	...
	if (argc < 2) {
		fprintf(stdout, "[ERROR] [RECEIVER] Need 2 arguments\n");
		return EXIT_FAILURE;
	}
	...
	if (file_specified) {
		struct stat fs;
		if (stat(fn, &fs) == 0) {
			fprintf(stdout, "[ERROR] [RECEIVER] File already exists: %s\n", fn);
			return EXIT_FAILURE;
		}
		f = fopen(fn, "a+");
		if (f == NULL) {
			fprintf(stdout, "[ERROR] [RECEIVER] Failed to open file: %s\n", fn);
		} else {
			fprintf(stdout, "[LOG] [RECEIVER] Opened file: %s\n", fn);
		}
	}
	...
	if (err) {
		fprintf(stdout, "[ERROR] [RECEIVER] Could not resolve hostname or IP: %s\n", err);
		return EXIT_FAILURE;
	}

	// bind to socket
	int sfd = create_socket(&addr, port, NULL, -1);
	fprintf(stdout, "Connected to socket %d\n", sfd);

	if (sfd > 0 && wait_for_client(sfd) < 0) {
		fprintf(stdout, "[ERROR] [RECEIVER] Could not connect to socket\n");
		close(sfd);
		return EXIT_FAILURE;
	}
	...
	if (file_specified) {
		fprintf(stdout, "[LOG] [RECEIVER] Ready to receive data\n");
		lastack = NULL;
		receive_data(f, sfd);
		fclose(f);
		free(lastack);
